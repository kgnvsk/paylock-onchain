...
/**
 * PayLock Escrow — Fund Escrow CLI
 * Usage: npx ts-node cli/fund.ts --escrow <PDA>
 */
...
async function main() {
  ...
  const clientAta = await getAssociatedTokenAddress(escrow.mint, wallet.publicKey);
  const [vaultPDA] = PublicKey.findProgramAddressSync(
    [escrowPDA.toBuffer(), TOKEN_PROGRAM_ID.toBuffer(), escrow.mint.toBuffer()],
    ASSOCIATED_TOKEN_PROGRAM_ID
  );

  console.log(`\n💰 Funding escrow ${escrowPDA.toString().slice(0, 8)}...`);
  console.log(`   Amount: ${escrow.amount.toString()} tokens\n`);

  const tx = await program.methods.fundEscrow().accounts({
